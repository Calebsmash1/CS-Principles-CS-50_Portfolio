...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    float blur_averageGreen;
        ...
        for (int i = 0; i < height; i++)
        {
            ...
            blur_averageRed = ((temp_image[i][j].rgbtRed + temp_image[i + 1][j + 1].rgbtRed + temp_image[i - 1][j - 1].rgbtRed) / 3);
            blur_averageBlue = ((temp_image[i][j].rgbtBlue + temp_image[i + 1][j + 1].rgbtBlue + temp_image[i - 1][j - 1].rgbtBlue) / 3);
            blur_averageGreen = ((temp_image[i][j].rgbtGreen + temp_image[i + 1][j + 1].rgbtGreen + temp_image[i - 1][j - 1].rgbtGreen) / 3);
